...
int combine_stacks(int nc, char* directory, char** filenames, float* weight, char* fileout);
...
int combine_slices(int nx, int ny, int nc, float* data_2D, float* weight, float* data_c) {
	...
	if (!tmp) {
		txbr_error(stderr, "ERROR: combine_slices - getting memory.\n");
		return MEMORY_ERROR;
	}

	int ix, iy, ic, index=0;
	...
	for (iy=0; iy<ny; iy++) {
		...
		for (ix=0; ix<nx; ix++) {
			...
			for (ic=0; ic<nc; ic++) {
				...
				tmp[index] = tmp[index] + weight[ic]*data_2D[ic*nx*ny+index];
	...
	memcpy(data_c, tmp, nx*ny*sizeof(float));
...
int main(int argc, char *argv[]) {
	...
	combine_stacks(nc, directory, filenames, weight, fileout);
