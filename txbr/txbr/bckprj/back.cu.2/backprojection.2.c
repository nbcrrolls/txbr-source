...
void initializeGPU( int n__, int number_of_terms_, int *order_in_X_, int *order_in_Y_, int *order_in_Z_,
					double *lambda__, double *a1__, double *a2__, double *bottom_plane__, double *rot__, int x_inc, int y_inc,
...
int evaluateBlock( float *block, int x0, int y0, int z0, int x_inc, int y_inc, int z_inc, int x1, int y1, int z1,
				   ...
				   int sliceMemSize, int blockMemSize, int itilt);
 ...
 */
void initialize_rotation_coefficients(double* plane_coefficients, double _rot_[3][3]) {
	...
	cos_theta = 1.0f/d2_sqr_root_;
	sin_theta = d1_sqr_root_/d2_sqr_root_;

	_rot_[0][0] = cos_phi*cos_phi*(cos_theta-1.0f) + 1.0f;	// _rot_[0][0]
	_rot_[0][1] = cos_phi*sin_phi*(cos_theta-1.0f);	// _rot_[0][1]
	_rot_[0][2] = -cos_phi*sin_theta;	// _rot_[0][2]

	_rot_[1][0] = cos_phi*sin_phi*(cos_theta-1.0f);	// _rot_[1][0]
	_rot_[1][1] = sin_phi*sin_phi*(cos_theta-1.0f) + 1.0f;	// _rot_[1][1]
	_rot_[1][2] = -sin_phi*sin_theta;	// _rot_[1][2]

	_rot_[2][0] = cos_phi*sin_theta;	// _rot_[2][0]
	_rot_[2][1] = sin_phi*sin_theta;	// _rot_[2][1]
	_rot_[2][2] = cos_theta ;	// _rot_[2][2]
...
int do_reconstruction( char* directory, char* basename, char* work_directory,
			Volume *vol, PathProjection* proj_map, int gpuDevId) {
	...
	if (blocksize_z>BLOCK_SIZE_MAX) {
		printf("Block size %i is higher than recommended %i.\n", 
	...
	memcpy((void *)bottom_plane_, (void *)setup->plane_coeffs2, 3 * sizeof(double));
...
//	printf("rot[1][0]=%4.5f\trot[1][1]=%4.5f\trot[1][2]=%4.5f\t\n",rot_[1][0],rot_[1][1],rot_[1][2]);
//	printf("rot[2][0]=%4.5f\trot[2][1]=%4.5f\trot[2][2]=%4.5f\t\n",rot_[2][0],rot_[2][1],rot_[2][2]);

	n_ = proj_map->order;
	number_of_terms = number_of_polynomial_terms(n_);

	init_power_order(n_, order_in_X, order_in_Y, order_in_Z);
	...
	int blockMemSize = nx_eff*ny_eff*blocksize_z*sizeof(float);
	float* block = (float*)malloc(blockMemSize);
	...
	init_float_array(block,nx_eff*ny_eff*blocksize_z);	/*	Initialization of the Output Image Buffer	*/
	...
	// This function inits the GPU hardware and preps some of the data structures
	// needed for the compute kernel
	initializeGPU(n_, number_of_terms, order_in_X, order_in_Y, order_in_Z, 
		lambda_, a1_, a2_, bottom_plane_, (double *)rot_, (int)setup->x_inc, (int)setup->y_inc,
		g_argc, g_argv, gpuDevId);

	int seg_size = nx_eff;

	seg_size = calculate_segment_size( proj_map->order, setup->x_0, setup->y_0, setup->z_0,
					setup->x_inc, setup->x_1, proj_map->coefficients_1, EPSILON);
	...
	int data_f_size = 2*nx_src*ny_src*sizeof(float);
	float* data_f = (float*)malloc(data_f_size);
	...
	if (!data_f) {
   	...
   	for (itilt=0; itilt<numtlts; itilt++) {
		...
		calculate_box(x_start, y_start, z_start, x_stop, y_stop, z_stop, nx, ny, 
			&x0_src, &y0_src, &nx_src, &ny_src, number_of_terms, 
			order_in_X, order_in_Y, order_in_Z, rot_, bottom_plane_, a1_, a2_);
			
		// printf("Box: origin: (%i,%i)  size:(%i,%i)\n",x0_src,y0_src,nx_src,ny_src);	
		
		if (data_f_size<nx_src*ny_src) {

			free(data_f);

			data_f_size = 2*nx_src*ny_src*sizeof(float);
			data_f = (float*)malloc(data_f_size);
		...
		initializeGPU( n_, number_of_terms, order_in_X, order_in_Y, order_in_Z,
					   lambda_, a1_, a2_, bottom_plane_, (double *)rot_, 
					   (int)setup->x_inc, (int)setup->y_inc,
					   g_argc, g_argv, gpuDevId);
		...
		evaluateBlock( block, x_start, y_start, z_start, x_inc, y_inc, z_inc, x_stop, y_stop, z_stop,
					   ...
					   data_f_size, blockMemSize, itilt);
	...
	// we are done with the GPU for now, free memory and cleanup
//	releaseGPUMemory();
	free(data_f);
	free(block);
...
int do_full_reconstruction(char* directory, char* basename, char* work_directory, TxBRsetup *setup, 
	PathProjection* proj_map, int gpuDevId ) {
	...
	int blocksize_x = (x_stop-x_start+1)/x_inc/2;
	int blocksize_y = (y_stop-y_start+1)/y_inc/2;
	...
	init_float_array(block_dest,nx_dest_f*ny_dest_f*blocksize_z);	/*	Initialization of the Output Image Buffer	*/
	...
	error = openMRCFile(work_directory, basename, input_file_header, &input_file);
	...
	for (iblock_z=0; iblock_z<numberOfZBlocks; iblock_z++) {  // block in the z direction
		...
		vol->z_stop = TXBR_MIN(vol->z_start + z_inc*(blocksize_z-1), z_stop-1);

		printf("Z block %i/%i. Starts at z=%f. Stops at z=%f\n", iblock_z+1, numberOfZBlocks, vol->z_start,vol->z_stop);
		...
		for (iblock_y=0; iblock_y<numberOfYBlocks; iblock_y++) {  // block in the y direction
			...
			for (iblock_x=0; iblock_x<numberOfXBlocks; iblock_x++) {  // block in the x direction
				...
				printf("X block %i/%i. Starts at x=%f. Stops at x=%f\n", iblock_x+1, numberOfXBlocks, vol->x_start, vol->x_stop);

				if ((error = do_reconstruction(directory, basename, work_directory, 
								vol, proj_map, gpuDevId)) != NO_ERROR) {
					return error;
				};
		...
		//save_block(work_directory, basename, z_start, iblock_z, nx_dest_f, ny_dest_f, blocksize_z, block_dest);
		save_block(work_directory, basename, z_start, iblock_z, nx_dest_f, ny_dest_f, vol->z_stop-vol->z_start+1, block_dest);
...
void embedBlock( float *block_dest, int x0_dest, int y0_dest, int nx_dest, int ny_dest,
	...
	for (i_=0; i_<nx_src; i_++) {
		x__ = delta_x + i_;
		if (x__>=nx_dest) continue;
		for (j_=0; j_<ny_src; j_++) {
			y__ = delta_y + j_;
			if (y__>=ny_dest) continue;
